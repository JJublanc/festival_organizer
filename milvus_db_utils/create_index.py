...
import os
import dotenv
from config import index_params, DATA_PATH
...
def get_args():
	...
	parser.add_argument('-e', '--embeded_field', type=str,
	                    default='etrange_festival_2023.csv',
	                    help='Name of the csv file containing the data')
...
def create_index(data: pd.DataFrame,
	...
	fields = [
		FieldSchema(name='id',
		            ...
		            is_primary=True,
		            auto_id=False),
		FieldSchema(name=embedded_field,
	...
	collection = create_an_empty_collection(fields=fields,
	                                        index_name=index_name,
	                                        index_param=index_param
	                                        )

	insert_each_title_and_its_embedding(data=data,
	                                    embedded_field=embedded_field,
	                                    collection=collection,
	                                    embedings=embedings)


def create_an_empty_collection(fields: list,
                               index_name: str,
                               index_param: dict
                               ) -> Collection:
...
def insert_each_title_and_its_embedding(data: pd.DataFrame,
                                        embedded_field: str,
                                        collection: Collection,
                                        embedings: list
                                        ) -> None:
