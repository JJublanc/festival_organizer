...
import time
from config import embedded_field
...
def search(text, index_param, collection):
	...
	index_name = f'embedded_field_{index_param["index_type"]}_{index_param["metric_type"]}'
	search_params = {
...
if __name__ == "__main__":
	...
	results = pd.DataFrame(
		columns=["id", "score", "embedded_field", "URL", "Title",
		         "Description", "search_query", "index_type", "metric_type"])
	search_queries = ["film d'horreur", "film érotique",
	...
	for search_query in search_queries:
		for i, index_param in enumerate(index_params):
			...
			for result in searches:
				...
				results = pd.concat([results, pd.DataFrame([{"id": result[0],
				                          "score": result[1],
				                          "embedded_field": result[2],
				                          "URL": data.loc[result[0], "URL"],
				                          "Title": data.loc[result[0], "Title"],
				                          "Description": data.loc[result[0],
				                          "Description"],
				                          "search_query": search_query,
				                          "index_type": index_params[i][
					                          "index_type"],
				                          "metric_type": index_params[i][
					                          "metric_type"],
				                          "delta_time_query": t}])], ignore_index=True)

	results.to_csv("./data/search_results.csv",
	               index=False)